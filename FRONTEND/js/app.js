...
const paginationState = {
    ...
    reservasAdmin: { page: 1, pageSize: 8 },
    paquetesAdmin: { page: 1, pageSize: 8 }
};
...
const renderGaleriaHabitacionDetalle = (imagenes, nombre) => {
    ...
    const galeriaHtml = lista.map((imagen, indice) => `
        <li><img src="${escaparHtml(imagen)}" alt="${escaparHtml(nombre || 'Habitación')} ${indice + 1}" onerror="this.src='assets/images/default.svg'"></li>
    `).join('');
...
function renderTablaReservasAdmin(lista) {
    ...
    const filtradas = lista.filter(r => {
        ...
        const coincide = !busqueda || cliente.includes(busqueda) || habitacion.includes(busqueda) || String(r.IDReserva).includes(busqueda);
        const estado = filtroEstado === 'all' || String(r.IdEstadoReserva) === filtroEstado;
        return coincide && estado;
    });

    const tablaWrap = tbody.closest('.crud-servicios-tabla-wrap') || tbody;
    ...
    if (filtradas.length === 0) {
        ...
        renderPaginationControls('reservasAdmin', tablaWrap, 0, 0, 1, () => renderTablaReservasAdmin(lista));
        return;
    }
    ...
    tbody.innerHTML = paginacion.items.map(r => {
        ...
        <tr>
            ...
            <td>
                <span>${escaparHtml(cliente)}</span><br>
                <small>${escaparHtml(r.NroDocumento || '')}</small>
            </td>
            <td>${escaparHtml(r.NombreHabitacion || '-')}</td>
            <td>
                <span>${fechaInicio}</span><br>
                <small>→ ${fechaFin}</small>
            </td>
            <td><strong>${total}</strong></td>
            <td>
                <label class="switch-estado" for="${escaparHtml(switchId)}" title="${switchLabel}">
                    <input id="${escaparHtml(switchId)}" type="checkbox"
    ...
    renderPaginationControls('reservasAdmin', tablaWrap, paginacion.totalItems, paginacion.totalPages, paginacion.currentPage, () => renderTablaReservasAdmin(lista));
}
...
async function cargarSelectsReserva(paquetesSeleccionados = [], serviciosSeleccionados = []) {
    ...
    if (gridPaquetes) {
        ...
        if (activos.length === 0) {
            gridPaquetes.innerHTML = '<p style="color:#64748b;font-size:.82rem;">No hay paquetes activos.</p>';
        } else {
            gridPaquetes.innerHTML = activos.map((p, idx) => {
                ...
                <div class="paquete-card ${seleccionado ? 'seleccionado' : ''}"
                    ...
                    <div class="paquete-card-img-wrap"><img src="${escaparHtml(imgSrc)}" class="paquete-card-img" alt="${escaparHtml(p.NombrePaquete || '')}" onerror="this.style.display='none'"></div>
    ...
    if (gridServicios) {
        ...
        if (activos.length === 0) {
            gridServicios.innerHTML = '<p style="color:#64748b;font-size:.82rem;">No hay servicios activos.</p>';
        } else {
...
function recalcularPrecio() {
    ...
    const sumaPaquetes = Array.from(document.querySelectorAll('#reserva-admin-paquetes-grid .paquete-card.seleccionado'))
        .reduce((sum, el) => sum + Number(el.dataset.precio || 0), 0);

    const sumaServicios = Array.from(document.querySelectorAll('#reserva-admin-servicios-grid .servicio-toggle-btn.seleccionado'))
...
function configurarCalculoPrecio() {
    ['reserva-admin-habitacion', 'reserva-admin-fecha-inicio', 'reserva-admin-fecha-fin'].forEach(id => {
...
async function guardarReservaAdmin(e) {
    ...
    const paquetesIds = Array.from(document.querySelectorAll('#reserva-admin-paquetes-grid .paquete-card.seleccionado[data-id]'))
        .map(el => Number(el.dataset.id));
    const serviciosIds = Array.from(document.querySelectorAll('#reserva-admin-servicios-grid .servicio-toggle-btn.seleccionado[data-id]'))
        .map(el => Number(el.dataset.id));
...
function configurarBuscadorCliente() {
    ...
    const mostrarSugerencias = (texto) => {
        ...
        if (coincidencias.length === 0) {
            lista.innerHTML = '<li style="padding:.52rem .88rem;font-size:.85rem;color:#64748b;">Sin resultados</li>';
        } else {
            lista.innerHTML = coincidencias.map(c => `
                <li class="autocomplete-item" data-id="${escaparHtml(String(c.IDCliente || c.NroDocumento))}"
                    ...
                    style="padding:.52rem .88rem;cursor:pointer;font-size:.85rem;color:#0f172a;">
                    <span>${escaparHtml(`${c.Nombre || ''} ${c.Apellido || ''}`.trim())}</span>
                    <small style="display:block;color:#64748b;">${escaparHtml(c.NroDocumento || '')}</small>
                </li>`).join('');
        }
    ...
    lista.addEventListener('click', (e) => {
        ...
        if (hiddenId) hiddenId.value = doc;
        input.dataset.documentoSeleccionado = doc;
        cerrarLista();
    });

    document.addEventListener('click', (e) => {
        if (!e.target.closest('.rf-auto-wrap') && !e.target.closest('.cliente-autocomplete')) cerrarLista();
    });
...
function mostrarDetalleReserva(reserva) {
    ...
    contenido.innerHTML = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
            <div><h5>Cliente</h5><p><strong>${escaparHtml(cliente || '-')}</strong></p><p>Doc: ${escaparHtml(reserva.NroDocumento || '-')}</p></div>
            <div><h5>Habitación</h5><p>${escaparHtml(reserva.NombreHabitacion || '-')}</p></div>
            <div><h5>Fechas</h5>
            ...
            <div><h5>Pago y estado</h5>
                <p>Método: ${escaparHtml(metodoPago)}</p>
                <p>Estado: ${estadoInfo.label}</p>
            </div>
            ${reserva.Paquetes ? `<div><h5>Paquetes</h5><p>${escaparHtml(reserva.Paquetes)}</p></div>` : ''}
            ${reserva.Servicios ? `<div><h5>Servicios</h5><p>${escaparHtml(reserva.Servicios)}</p></div>` : ''}
        </div>`;
...
function configurarCRUDReservas() {
    ...
    if (busqueda && !busqueda.dataset.inicializado) {
        busqueda.addEventListener('input', () => { resetPagination('reservasAdmin'); renderTablaReservasAdmin(reservasCargadas); });
    ...
    if (filtroEstado && !filtroEstado.dataset.inicializado) {
        filtroEstado.addEventListener('change', () => { resetPagination('reservasAdmin'); renderTablaReservasAdmin(reservasCargadas); });
        filtroEstado.dataset.inicializado = 'true';
    }
}

// ============================================
// FUNCIONES PAQUETES ADMIN
// ============================================

function renderizarPaquetesAdmin() {
    const tbody = document.getElementById('paquetes-admin-tbody');
    if (!tbody) return;

    const busqueda = normalizarTexto(document.getElementById('busqueda-paquetes-admin')?.value || '');
    const filtroEstado = document.getElementById('filtro-estado-paquetes-admin')?.value || 'all';

    let lista = paquetesCargados.filter((p) => {
        const texto = `${p.NombrePaquete || ''} ${p.NombreHabitacion || ''} ${p.NombreServicio || ''}`.toLowerCase();
        const pasaBusqueda = !busqueda || texto.includes(busqueda);
        const pasaFiltro = filtroEstado === 'all' || String(p.Estado) === filtroEstado;
    ...
    const totalEl = document.getElementById('paquetes-admin-total');
    const activosEl = document.getElementById('paquetes-admin-activos');
    const inactivosEl = document.getElementById('paquetes-admin-inactivos');
    if (totalEl) totalEl.textContent = total;
    if (activosEl) activosEl.textContent = activos;
    if (inactivosEl) inactivosEl.textContent = inactivos;

    if (lista.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="mensaje-vacio">No se encontraron paquetes</td></tr>';
        return;
    }

    const { items, totalItems, totalPages, currentPage } = getPaginatedItems(lista, 'paquetesAdmin');

    tbody.innerHTML = items.map((p) => `
        <tr>
            <td>${p.NombrePaquete || '-'}</td>
            <td>${p.NombreHabitacion || p.IDHabitacion || '-'}</td>
            <td>${p.NombreServicio || p.IDServicio || '-'}</td>
            <td>${formatearMonto(p.Precio)}</td>
            <td>${Number(p.Estado) === 1 ? 'Activo' : 'Inactivo'}</td>
            <td class="crud-acciones">
                <button class="btn-accion-crud btn-editar" onclick="editarPaqueteAdmin(${p.IDPaquete})" title="Editar">
                    <i class="fa-solid fa-pen"></i>
                </button>
                <button class="btn-accion-crud btn-eliminar" onclick="eliminarPaqueteAdmin(${p.IDPaquete})" title="Eliminar">
    ...
    renderPaginationControls('paquetesAdmin', tbody, totalItems, totalPages, currentPage, renderizarPaquetesAdmin);
}
...
async function cargarPaquetesAdmin() {
    ...
    try {
        ...
        renderizarPaquetesAdmin();
        configurarPaquetesAdmin();
    } catch (error) {
        console.error('Error cargando paquetes:', error);
        paquetesCargados = [];
        tbody.innerHTML = '<tr><td colspan="6" class="mensaje-vacio">Error al cargar paquetes</td></tr>';
    }
}

function configurarPaquetesAdmin() {
    ...
    const filtro = document.getElementById('filtro-estado-paquetes-admin');

    if (btnNuevo && !btnNuevo.dataset.init) {
        btnNuevo.addEventListener('click', () => abrirModalPaqueteAdmin());
        btnNuevo.dataset.init = '1';
    }
    if (form && !form.dataset.init) {
        form.addEventListener('submit', guardarPaqueteAdmin);
        form.dataset.init = '1';
    }
    if (btnCerrar && !btnCerrar.dataset.init) {
        btnCerrar.addEventListener('click', cerrarModalPaqueteAdmin);
        btnCerrar.dataset.init = '1';
    }
    if (btnLimpiar && !btnLimpiar.dataset.init) {
        btnLimpiar.addEventListener('click', () => limpiarFormPaquete());
        btnLimpiar.dataset.init = '1';
    }
    if (busqueda && !busqueda.dataset.init) {
        busqueda.addEventListener('input', () => { resetPagination('paquetesAdmin'); renderizarPaquetesAdmin(); });
        busqueda.dataset.init = '1';
    }
    if (filtro && !filtro.dataset.init) {
        filtro.addEventListener('change', () => { resetPagination('paquetesAdmin'); renderizarPaquetesAdmin(); });
        filtro.dataset.init = '1';
    }
}
...
function abrirModalPaqueteAdmin(paquete = null) {
    ...
    if (paquete) {
        ...
        document.getElementById('paquete-admin-habitacion').value = paquete.IDHabitacion || '';
        document.getElementById('paquete-admin-servicio').value = paquete.IDServicio || '';
        ...
        document.getElementById('paquete-admin-cliente').value = paquete.IDCliente || '';
        document.getElementById('paquete-admin-imagen').value = paquete.ImagenURL || '';
    } else {
...
function limpiarFormPaquete() {
    const form = document.getElementById('form-paquete-admin');
    if (form) form.reset();
    const id = document.getElementById('paquete-admin-id');
    if (id) id.value = '';
    const msg = document.getElementById('mensaje-paquete-admin-modal');
    if (msg) msg.textContent = '';
}

async function guardarPaqueteAdmin(evento) {
    evento.preventDefault();
    const idActual = document.getElementById('paquete-admin-id')?.value?.trim();
    const btn = document.getElementById('btn-paquete-admin-guardar');
    const msgEl = document.getElementById('mensaje-paquete-admin-modal');

    const clienteVal = document.getElementById('paquete-admin-cliente')?.value?.trim();
    const payload = {
        NombrePaquete: document.getElementById('paquete-admin-nombre')?.value?.trim(),
        Descripcion: document.getElementById('paquete-admin-descripcion')?.value?.trim(),
        IDHabitacion: Number(document.getElementById('paquete-admin-habitacion')?.value),
        IDServicio: Number(document.getElementById('paquete-admin-servicio')?.value),
        Precio: Number(document.getElementById('paquete-admin-precio')?.value),
        Estado: Number(document.getElementById('paquete-admin-estado')?.value),
        IDCliente: clienteVal ? Number(clienteVal) : null,
        ImagenURL: document.getElementById('paquete-admin-imagen')?.value?.trim() || null,
    };

    if (!payload.NombrePaquete || !payload.IDHabitacion || !payload.IDServicio || !payload.Precio) {
        if (msgEl) msgEl.textContent = 'Completa los campos obligatorios.';
        return;
    }
    ...
    try {
        if (idActual) {
            await actualizarPaquete(idActual, payload);
        } else {
            await crearPaquete(payload);
        }
        cerrarModalPaqueteAdmin();
        await cargarPaquetesAdmin();
    } catch (error) {
        if (msgEl) msgEl.textContent = error.message || 'No se pudo guardar el paquete.';
    } finally {
        if (btn) btn.disabled = false;
    }
}

async function editarPaqueteAdmin(id) {
    const paquete = paquetesCargados.find((p) => String(p.IDPaquete) === String(id));
    if (paquete) abrirModalPaqueteAdmin(paquete);
}
...
async function cargarDashboardAdmin() {
    const totalEl = document.getElementById('dash-total-reservas');
    const ingresosEl = document.getElementById('dash-ingresos-totales');
    const habTbody = document.getElementById('dash-habitaciones-tbody');
    const servTbody = document.getElementById('dash-servicios-tbody');

    if (!totalEl) return;

    if (totalEl) totalEl.textContent = 'Cargando...';
    if (ingresosEl) ingresosEl.textContent = 'Cargando...';
    ...
    if (!data) {
        ...
        if (habTbody) habTbody.innerHTML = '<tr><td colspan="2" class="mensaje-vacio">No disponible</td></tr>';
        if (servTbody) servTbody.innerHTML = '<tr><td colspan="2" class="mensaje-vacio">No disponible</td></tr>';
        return;
    }

    if (totalEl) totalEl.textContent = data.totalReservas ?? 0;
    if (ingresosEl) ingresosEl.textContent = formatearMonto(data.ingresosTotales ?? 0);

    if (habTbody) {
        const habs = Array.isArray(data.habitacionesMasReservadas) ? data.habitacionesMasReservadas : [];
        habTbody.innerHTML = habs.length
            ? habs.map((h) => `<tr><td>${h.NombreHabitacion}</td><td>${h.total}</td></tr>`).join('')
            : '<tr><td colspan="2" class="mensaje-vacio">Sin datos</td></tr>';
    }

    if (servTbody) {
        const servs = Array.isArray(data.serviciosMasVendidos) ? data.serviciosMasVendidos : [];
        servTbody.innerHTML = servs.length
            ? servs.map((s) => `<tr><td>${s.NombreServicio}</td><td>${s.total}</td></tr>`).join('')
            : '<tr><td colspan="2" class="mensaje-vacio">Sin datos</td></tr>';
    }
}
...
window.cargarSelectsReserva = cargarSelectsReserva;
window.recalcularPrecio = recalcularPrecio;
window.limpiarFormularioReservaAdmin = limpiarFormularioReservaAdmin;
window.guardarReservaAdmin = guardarReservaAdmin;
window.configurarBuscadorCliente = configurarBuscadorCliente;
window.editarPaqueteAdmin = editarPaqueteAdmin;
