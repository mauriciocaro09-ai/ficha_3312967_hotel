...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cerrar todos los modales y el sidebar con Escape
    document.addEventListener('keydown', (e) => {
        if (e.key !== 'Escape') return;

        if (typeof cerrarModalesCRUD === 'function') cerrarModalesCRUD();
        if (typeof cerrarDetalleAdmin === 'function') cerrarDetalleAdmin();

        const sidebar = document.getElementById('sidebar');
        if (sidebar?.classList.contains('open') && typeof closeSidebar === 'function') {
            closeSidebar();
        }
    });
