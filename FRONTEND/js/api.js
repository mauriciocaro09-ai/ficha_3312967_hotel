...
async function actualizarPaquete(id, paquete) {
    apiLogger.log('Actualizando paquete:', id, paquete);
    try {
...
async function obtenerEstadisticasDashboard() {
    apiLogger.log('Obteniendo estadísticas del dashboard...');
    try {
        return await requestJson('/dashboard/estadisticas');
    } catch (error) {
        apiLogger.error('Error al obtener estadísticas:', error.message);
        return null;
    }
}
